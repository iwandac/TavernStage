...
基于 [SillyTavern](https://github.com/SillyTavern/SillyTavern) 演进的角色扮演智能体运行时项目。目标是保留社区积累的角色卡、世界书、提示词编排与模型适配能力，将它们从交互式宿主中抽取为可被其他产品调用、可持续同步上游的运行时。
...
- 待实现：无头调用接口、角色扮演逻辑抽取、会话隔离与恢复、酒馆角色卡接入。
- 未承诺：Cloudflare Workers 直接运行、完整扩展兼容、生产可用性或高并发指标。
