...
- 待实现与验证：云端生产权威存储/部署适配、其它供应商的 Node 宿主适配和完整扩展生态。
...
宿主的版本锁必须同时区分**交付提交**与**运行源码版本**。例如，仅更新本 README 的后继提交仍可使用 `612d64d6ed74ac6378248e63671eaab77cc53c07` 的已验证运行源码身份，但必须实际比较源树和依赖锁，不能只相信版本字符串；代码、配置或锁有变化就需要重新验收和明确会话兼容策略。未知 capsule/profile 不应被旧版本强行读取，更不能通过删除用户会话来让回退检查通过。

2026-09-05 核对官方正式发行：最新仍为 [1.18.0](https://github.com/SillyTavern/SillyTavern/releases/tag/1.18.0)，标签提交 `51ad27fb86d39a3daca3adaa970375c9670c12df` 已包含在采用基线 `8172dcd0ee672d3cd9a5e5f7af134f91a45cd2b8`；官方 `release` 也仍指向该基线。因此本轮没有可采用的新正式版。历史局部修复迁移不是完整发行升级证明，下一正式升级仍须执行下面的合并、回归与回退流程。
