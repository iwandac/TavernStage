// TavernStage shared core, extracted from public/scripts/macros/engine/MacroEngine.js.
// Upstream 8172dcd0ee672d3cd9a5e5f7af134f91a45cd2b8; AGPL-3.0; source declarations: MacroEngine:36
...
class MacroEngine {
    ...
    evaluate(input, env, { contextOffset = 0 } = {}) {
        ...
        const { cst, lexingErrors, parserErrors } = __stage.MacroParser.parseDocument(preProcessed);

        // For now, we log and still try to process what we can.
        if (lexingErrors && lexingErrors.length > 0) {
            (0, __stage.logMacroSyntaxWarning)({ phase: 'lexing', input, errors: lexingErrors });
        }
        if (parserErrors && parserErrors.length > 0) {
            (0, __stage.logMacroSyntaxWarning)({ phase: 'parsing', input, errors: parserErrors });
        }

        // If the parser did not produce a valid CST, fall back to the original input.
        if (!cst || typeof cst !== 'object' || !cst.children) {
            (0, __stage.logMacroGeneralError)({ message: 'Macro parser produced an invalid CST. Returning original input.', error: { input, lexingErrors, parserErrors } });
            return input;
        }

        let evaluated;
        try {
            evaluated = __stage.MacroCstWalker.evaluateDocument({
                text: preProcessed,
                contextOffset,
                cst,
                env: safeEnv,
            ...
            (0, __stage.logMacroGeneralError)({ message: 'Macro evaluation failed. Returning original input.', error: { input, error } });
            return input;
        }
        ...
        if (Object.hasOwn(env.dynamicMacros, nameLower)) {
            ...
            if (looksLikeOptions) {
                ...
                try {
                    ...
                    defOverride = __stage.MacroRegistry.buildMacroDefFromOptions(name, options);
                } catch (error) {
                    // If building fails, log warning and fall through to check registered macros
                    (0, __stage.logMacroRuntimeWarning)({ message: `Dynamic macro "${name}" has invalid options: ${error.message}`, call });
                }
                ...
                if (['number', 'boolean'].includes(typeof impl)) {
                    (0, __stage.logMacroRuntimeWarning)({ message: `Dynamic macro "${name}" uses unsupported number/boolean format.`, call });
                }
                defOverride = __stage.MacroRegistry.buildMacroDefFromOptions(name, {
                ...
                (0, __stage.logMacroRuntimeWarning)({ message: `Dynamic macro "${name}" is not defined correctly (must be string, a handler function, or a macro def options object with handler property).`, call });
            }
        }

        // If not, check if the macro exists and is registered
        if (!defOverride && !__stage.MacroRegistry.hasMacro(name)) {
            return raw; // Unknown macro: keep macro syntax, but nested macros inside rawInner are already resolved.
        }

        try {
            const result = __stage.MacroRegistry.executeMacro(call, { defOverride });

            try {
                return call.env.functions.postProcess(result);
            } catch (error) {
                (0, __stage.logMacroInternalError)({ message: `Macro "${name}" postProcess function failed.`, call, error });
                return result;
            }
        } catch (error) {
            const isRuntimeError = !!(error && (error.name === 'MacroRuntimeError' || error.isMacroRuntimeError));
            if (isRuntimeError) {
                (0, __stage.logMacroRuntimeWarning)({ message: (error.message || `Macro "${name}" execution failed.`), call, error });
            } else {
                (0, __stage.logMacroInternalError)({ message: `Macro "${name}" internal execution error.`, call, error });
            }
            return raw;
        }
    }
    ...
    #registerCorePostProcessors() {
        ...
        this.addPostProcessor(
            text => text.replaceAll(__stage.ELSE_MARKER, ''),
