// TavernStage shared core, extracted from public/scripts/PromptManager.js.
...
class PromptManager {
    get promptSources() {
        return {
            charDescription: (0, __stage.t)`Character Description`,
            charPersonality: (0, __stage.t)`Character Personality`,
            scenario: (0, __stage.t)`Character Scenario`,
            personaDescription: (0, __stage.t)`Persona Description`,
            worldInfoBefore: (0, __stage.t)`World Info (↑Char)`,
            worldInfoAfter: (0, __stage.t)`World Info (↓Char)`,
        };
    }
    ...
    constructor() {
        ...
        this.renderDebounced = (0, __stage.debounce)(this.render.bind(this), __stage.debounce_timeout.relaxed);
    }
    ...
    init(moduleConfiguration, serviceSettings) {
        ...
        this.tokenHandler = this.tokenHandler || new __stage.TokenHandler(() => { throw new Error('Token handler not set'); });
        this.serviceSettings = serviceSettings;
        this.containerElement = __stage.document.getElementById(this.configuration.containerIdentifier);
        ...
        this.handleResetPrompt = (event) => {
            ...
            const nameField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_name'));
            const roleField = /** @type {HTMLSelectElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_role'));
            const promptField = /** @type {HTMLTextAreaElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_prompt'));
            const injectionPositionField = /** @type {HTMLSelectElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_position'));
            const injectionDepthField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_depth'));
            const injectionOrderField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_order'));
            const injectionTriggerField = /** @type {HTMLSelectElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_trigger'));
            const depthBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_depth_block'));
            const orderBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_order_block'));
            const forbidOverridesField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_forbid_overrides'));
            const forbidOverridesBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_forbid_overrides_block'));
            const entrySourceBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source_block'));
            const entrySource = /** @type {HTMLSpanElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source'));
            ...
            injectionTriggerField.dispatchEvent(new __stage.Event('change', { bubbles: true }));
        ...
        this.handleAppendPrompt = (event) => {
            const appendPromptFooter = /** @type {HTMLSelectElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_footer_append_prompt'));
        ...
        this.handleDeletePrompt = async (event) => {
            __stage.Popup.show.confirm((0, __stage.t)`Are you sure you want to delete this prompt?`, null).then((userChoice) => {
        ...
        this.handleImport = () => {
            __stage.Popup.show.confirm((0, __stage.t)`Existing prompts with the same ID will be overridden. Do you want to proceed?`, null)
                .then(userChoice => {
                    if (!userChoice) return;

                    const fileOpener = __stage.document.createElement('input');
                    fileOpener.type = 'file';
                    fileOpener.accept = '.json';

                    fileOpener.addEventListener('change', (event) => {
                        if (!(event.target instanceof __stage.HTMLInputElement)) return;
                        const file = event.target.files[0];
                        if (!file) return;

                        const reader = new __stage.FileReader();
                        ...
                        reader.onload = (event) => {
                            ...
                            try {
                                ...
                                __stage.toastr.error((0, __stage.t)`An error occurred while importing prompts. More info available in console.`);
                                __stage.console.log('An error occurred while importing prompts');
                                __stage.console.log(err.toString());
                            }
                        };
        ...
        this.handleCharacterReset = () => {
            __stage.Popup.show.confirm((0, __stage.t)`This will reset the prompt order for this character. You will not lose any prompts.`, null)
                .then(userChoice => {
                    if (!userChoice) return;
        ...
        if ('global' === this.configuration.promptOrder.strategy) {
            const handleQuickEditSave = (event) => {
                ...
                const popupEditFormPrompt = /** @type {HTMLTextAreaElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_prompt'));
            ...
            __stage.document.getElementById(mainElementId).addEventListener('blur', handleQuickEditSave);

            const nsfwPrompt = this.getPromptById('nsfw');
            const nsfwElementId = this.updateQuickEdit('nsfw', nsfwPrompt);
            __stage.document.getElementById(nsfwElementId).addEventListener('blur', handleQuickEditSave);
            ...
            __stage.document.getElementById(jailbreakElementId).addEventListener('blur', handleQuickEditSave);
        }

        // Re-render when chat history changes.
        __stage.eventSource.on(__stage.event_types.MESSAGE_DELETED, () => this.renderDebounced());
        __stage.eventSource.on(__stage.event_types.MESSAGE_EDITED, () => this.renderDebounced());
        __stage.eventSource.on(__stage.event_types.MESSAGE_RECEIVED, () => this.renderDebounced());

        // Re-render when chatcompletion settings change
        __stage.eventSource.on(__stage.event_types.CHATCOMPLETION_SOURCE_CHANGED, () => this.renderDebounced());

        __stage.eventSource.on(__stage.event_types.CHATCOMPLETION_MODEL_CHANGED, () => this.renderDebounced());

        // Re-render when the character changes.
        __stage.eventSource.on(__stage.event_types.CHAT_LOADED, (event) => {
        ...
        __stage.eventSource.on(__stage.event_types.CHARACTER_EDITED, (event) => {
            this.handleCharacterUpdated(event);
            this.saveServiceSettings().then(() => this.renderDebounced());
        });

        // Re-render when the group changes.
        __stage.eventSource.on('groupSelected', (event) => {
            this.handleGroupSelected(event);
            this.saveServiceSettings().then(() => this.renderDebounced());
        });

        // Sanitize settings after character has been deleted.
        __stage.eventSource.on(__stage.event_types.CHARACTER_DELETED, (event) => {
            this.handleCharacterDeleted(event);
            this.saveServiceSettings().then(() => this.renderDebounced());
        });

        // Trigger re-render when token settings are changed
        __stage.document.getElementById('openai_max_context').addEventListener('change', (event) => {
            if (!(event.target instanceof __stage.HTMLInputElement)) return;
            this.serviceSettings.openai_max_context = event.target.value;
            if (this.activeCharacter) this.renderDebounced();
        });

        __stage.document.getElementById('openai_max_tokens').addEventListener('change', (event) => {
            if (this.activeCharacter) this.renderDebounced();
        });

        // Prepare prompt edit form buttons
        __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_save').addEventListener('click', this.handleSavePrompt);
        __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_reset').addEventListener('click', this.handleResetPrompt);
        ...
        __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_close').addEventListener('click', closeAndClearPopup);
        __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_close_button').addEventListener('click', closeAndClearPopup);
        closeAndClearPopup();

        // Re-render prompt manager on openai preset change
        __stage.eventSource.on(__stage.event_types.OAI_PRESET_CHANGED_AFTER, () => {
            this.sanitizeServiceSettings();
            const mainPrompt = this.getPromptById('main');
        ...
        __stage.eventSource.on(__stage.event_types.WORLDINFO_SETTINGS_UPDATED, () => this.renderDebounced());
     ...
     */
    #getScrollPosition() {
        return __stage.document.getElementById(this.configuration.prefix + 'prompt_manager')?.closest('.scrollableInner')?.scrollTop;
    }
    ...
    #setScrollPosition(scrollPosition) {
        ...
        __stage.document.getElementById(this.configuration.prefix + 'prompt_manager')?.closest('.scrollableInner')?.scrollTo(0, scrollPosition);
    }
    ...
    render(afterTryGenerate = true) {
        if (__stage.main_api !== 'openai') return;

        if ('character' === this.configuration.promptOrder.strategy && null === this.activeCharacter) return;
        this.error = null;

        (0, __stage.waitUntilCondition)(() => !__stage.is_send_press && !__stage.is_group_generating, 1024 * 1024, 100).then(async () => {
            ...
            __stage.console.log('Timeout while waiting for send press to be false');
        });
    }
    ...
    preparePrompt(prompt, original = null) {
        ...
        if (typeof original === 'string') {
            if (0 < groupMembers.length) preparedPrompt.content = (0, __stage.substituteParams)(prompt.content ?? '', { original, groupOverride: groupMembers.join(', ') });
            else preparedPrompt.content = (0, __stage.substituteParams)(prompt.content, { original });
        } else {
            if (0 < groupMembers.length) preparedPrompt.content = (0, __stage.substituteParams)(prompt.content ?? '', { groupOverride: groupMembers.join(', ') });
            else preparedPrompt.content = (0, __stage.substituteParams)(prompt.content);
        }
    ...
    createQuickEdit(identifier, title) {
        ...
        const quickEditContainer = __stage.document.getElementById('quick-edit-container');
        quickEditContainer.insertAdjacentHTML('afterbegin', html);

        const debouncedSaveServiceSettings = (0, __stage.debouncePromise)(() => this.saveServiceSettings(), 300);

        const textarea = /** @type {HTMLTextAreaElement} */(__stage.document.getElementById(textareaIdentifier));
    ...
    updateQuickEdit(identifier, prompt) {
        ...
        const textarea = /** @type {HTMLTextAreaElement} */(__stage.document.getElementById(elementId));
    ...
    loadPromptIntoEditForm(prompt) {
        ...
        const injectionOrderField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_order'));
        const injectionTriggerField = /** @type {HTMLSelectElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_injection_trigger'));
        const injectionDepthBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_depth_block'));
        const injectionOrderBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_order_block'));
        const forbidOverridesField = /** @type {HTMLInputElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_forbid_overrides'));
        const forbidOverridesBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_forbid_overrides_block'));
        const entrySourceBlock = /** @type {HTMLDivElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source_block'));
        const entrySource = /** @type {HTMLSpanElement} */(__stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_source'));
        ...
        const resetPromptButton = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_reset');
        ...
        const savePromptButton = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_save');
        savePromptButton.dataset.pmPrompt = prompt.identifier;
    }

    handleInjectionPositionChange(event) {
        const injectionDepthBlock = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_depth_block');
        const injectionOrderBlock = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_order_block');
    ...
    loadMessagesIntoInspectForm(messages) {
        ...
        <div class="inline-drawer ${this.configuration.prefix}prompt_manager_prompt">
            <div class="inline-drawer-toggle inline-drawer-header">
                <span>Name: ${(0, __stage.escapeHtml)(title)}, Role: ${role}, Tokens: ${tokens}</span>
                <div class="fa-solid fa-circle-chevron-down inline-drawer-icon down"></div>
            </div>
            <div class="inline-drawer-content" style="white-space: pre-wrap;">${(0, __stage.escapeHtml)(content)}</div>
        </div>
        `;

            let template = __stage.document.createElement('template');
            template.innerHTML = drawerHTML.trim();
            return template.content.firstChild;
        };

        const messageList = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_entry_form_inspect_list');

        const messagesCollection = messages instanceof __stage.Message ? [messages] : messages.getCollection();
     ...
     */
    clearEditForm() {
        const editArea = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_edit');
    ...
    clearInspectForm() {
        const inspectArea = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_inspect');
    ...
    getPromptCollection(generationType) {
        ...
        promptOrder.forEach(entry => {
            ...
            if (allowedTrigger) {
                ...
                const replacementPrompt = (0, __stage.structuredClone)(prompt);
    ...
    async renderPromptManager() {
        ...
        const existingAppendSelect = __stage.document.getElementById(`${this.configuration.prefix}prompt_manager_footer_append_prompt`);
        if (existingAppendSelect instanceof __stage.HTMLSelectElement) {
        ...
        const errorDiv = this.error ? `
                <div class="${this.configuration.prefix}prompt_manager_error">
                    <span class="fa-solid tooltip fa-triangle-exclamation text_danger"></span> ${__stage.DOMPurify.sanitize(this.error)}
                </div>
        ` : '';

        const totalActiveTokens = this.tokenUsage;

        const headerHtml = await (0, __stage.renderTemplateAsync)('promptManagerHeader', { error: this.error, errorDiv, prefix: this.configuration.prefix, totalActiveTokens });
        ...
        if (null !== this.activeCharacter) {
            ...
            const promptsHtml = prompts.reduce((acc, prompt) => acc + `<option value="${prompt.identifier}">${(0, __stage.escapeHtml)(prompt.name)}</option>`, '');

            if (selectedPromptIndex > 0) {
                selectedPromptIndex = __stage.Math.min(selectedPromptIndex, prompts.length - 1);
            }
            ...
            const footerHtml = await (0, __stage.renderTemplateAsync)('promptManagerFooter', { promptsHtml, prefix: this.configuration.prefix });
    ...
    async renderPromptManagerListItems() {
        ...
        let listItemHtml = await (0, __stage.renderTemplateAsync)('promptManagerListHeader', { prefix });
        ...
        this.getPromptsForCharacter(this.activeCharacter).forEach(prompt => {
            ...
            const encodedName = (0, __stage.escapeHtml)(prompt.name);
            ...
            listItemHtml += `
                <li class="${prefix}prompt_manager_prompt ${draggableClass} ${enabledClass} ${markerClass} ${importantClass}" data-pm-identifier="${(0, __stage.escapeHtml)(prompt.identifier)}">
                    ...
                    <span class="${prefix}prompt_manager_prompt_name" data-pm-name="${encodedName}">
                        ...
                        ${roleIcon ? `<span data-role="${(0, __stage.escapeHtml)(prompt.role)}" class="fa-xs fa-solid ${roleIcon}" title="${roleTitle}"></span>` : ''}
                        ${isInjectionPrompt ? `<small class="prompt-manager-injection-depth">@ ${(0, __stage.escapeHtml)(prompt.injection_depth.toString())}</small>` : ''}
    ...
    export(data, type, name = 'export') {
        ...
        const blob = new __stage.Blob([serializedObject], { type: 'application/json' });
        const url = URL.createObjectURL(blob);
        const downloadLink = __stage.document.createElement('a');
    ...
    import(importData) {
        ...
        if (false === this.validateObject(controlObj, importData)) {
            __stage.toastr.warning((0, __stage.t)`Could not import prompts. Export failed validation.`);
            return;
        }
        ...
        __stage.toastr.success((0, __stage.t)`Prompt import complete.`);
        this.saveServiceSettings().then(() => this.render());
    }
     ...
     */
    makeDraggable() {
        (0, __stage.$)(`#${this.configuration.prefix}prompt_manager_list`).sortable({
            delay: this.configuration.sortableDelay,
            handle: (0, __stage.isMobile)() ? '.drag-handle' : null,
            items: `.${this.configuration.prefix}prompt_manager_prompt_draggable`,
            update: (event, ui) => {
                const promptOrder = this.getPromptOrderForCharacter(this.activeCharacter);
                const promptListElement = (0, __stage.$)(`#${this.configuration.prefix}prompt_manager_list`).sortable('toArray', { attribute: 'data-pm-identifier' });
     ...
     */
    showPopup(area = 'edit') {
        const areaElement = __stage.document.getElementById(this.configuration.prefix + 'prompt_manager_popup_' + area);
        areaElement.style.display = 'flex';

        (0, __stage.$)('#' + this.configuration.prefix + 'prompt_manager_popup').first()
            .slideDown(200, 'swing')
            .addClass('openDrawer');
    }
     ...
     */
    log(output) {
        if (__stage.power_user.console_log_prompts) __stage.console.log('[PromptManager] ' + output);
    }
    ...
    profileStart(identifier) {
        if (__stage.power_user.console_log_prompts) __stage.console.time(identifier);
    }
    ...
    profileEnd(identifier) {
        if (__stage.power_user.console_log_prompts) {
            ...
            __stage.console.timeEnd(identifier);
        }
    }
}
...
return { DEFAULT_DEPTH, DEFAULT_ORDER, INJECTION_POSITION, Prompt, PromptCollection, PromptManager, chatCompletionDefaultPrompts, promptManagerDefaultPromptOrders, promptManagerDefaultPromptOrder };
}
