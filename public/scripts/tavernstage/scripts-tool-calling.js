// TavernStage shared core, extracted from public/scripts/tool-calling.js.
...
class ToolManager {
    ...
    static registerFunctionTool({ name, displayName, description, parameters, action, formatMessage, shouldRegister, stealth }) {
        ...
        if (this.#tools.has(name)) {
            __stage.console.warn(`[ToolManager] A tool with the name "${name}" has already been registered. The definition will be overwritten.`);
        }
        ...
        __stage.console.log('[ToolManager] Registered function tool:', definition);
    }
    ...
    static unregisterFunctionTool(name) {
        ...
        __stage.console.log(`[ToolManager] Unregistered function tool: ${name}`);
    }
    ...
    */
    ...
    static async invokeFunctionTool(name, parameters) {
        try {
            ...
            __stage.console.error(`[ToolManager] An error occurred while invoking the tool "${name}":`, error);
    ...
    static async formatToolCallMessage(name, parameters) {
        ...
        try {
            ...
            __stage.console.error(`[ToolManager] An error occurred while formatting the tool call message for "${name}":`, error);
    ...
    static async registerFunctionToolsOpenAI(data) {
        ...
        for (const tool of ToolManager.tools) {
            ...
            if (!register) {
                __stage.console.log('[ToolManager] Skipping tool registration:', tool);
                continue;
            }
        ...
        if (tools.length) {
            __stage.console.log('[ToolManager] Registered function tools:', tools);
            ...
            if (targetToolCall) {
                ...
                if (jsonDeltaString) {
                    try {
                        ...
                        __stage.console.warn('[ToolManager] Failed to apply input JSON delta:', error);
                    }
                }
            }
        }
    ...
    static isToolCallingSupported(settings = null, model = null) {
        ...
        model = model ?? (0, __stage.getChatCompletionModel)(settings);

        if (__stage.main_api !== 'openai' || !settings.function_calling) {
            return false;
        }

        // Post-processing will forcefully remove past tool calls from the prompt, making them useless
        const { NONE, MERGE_TOOLS, SEMI_TOOLS, STRICT_TOOLS } = __stage.custom_prompt_post_processing_types;
        ...
        const currentModel = Array.isArray(__stage.model_list) ? __stage.model_list.find(m => m.id === model) : null;
        if (currentModel) {
            switch (settings.chat_completion_source) {
                case __stage.chat_completion_sources.POLLINATIONS:
                    return currentModel.tools;
                case __stage.chat_completion_sources.FIREWORKS:
        ...
        const supportedSources = [
            __stage.chat_completion_sources.OPENAI,
            __stage.chat_completion_sources.CUSTOM,
            __stage.chat_completion_sources.MISTRALAI,
            __stage.chat_completion_sources.CLAUDE,
            __stage.chat_completion_sources.OPENROUTER,
            __stage.chat_completion_sources.AIMLAPI,
            __stage.chat_completion_sources.GROQ,
            __stage.chat_completion_sources.COHERE,
            __stage.chat_completion_sources.DEEPSEEK,
            __stage.chat_completion_sources.MAKERSUITE,
            __stage.chat_completion_sources.VERTEXAI,
            __stage.chat_completion_sources.AI21,
            __stage.chat_completion_sources.XAI,
            __stage.chat_completion_sources.POLLINATIONS,
            __stage.chat_completion_sources.MOONSHOT,
            __stage.chat_completion_sources.FIREWORKS,
            __stage.chat_completion_sources.COMETAPI,
            __stage.chat_completion_sources.CHUTES,
            __stage.chat_completion_sources.ELECTRONHUB,
            __stage.chat_completion_sources.AZURE_OPENAI,
            __stage.chat_completion_sources.ZAI,
            __stage.chat_completion_sources.SILICONFLOW,
            __stage.chat_completion_sources.NANOGPT,
            __stage.chat_completion_sources.WORKERS_AI,
            __stage.chat_completion_sources.MINIMAX,
        ];
    ...
    static #getToolCallsFromData(data) {
        const getRandomId = () => __stage.Math.random().toString(36).substring(2);
    ...
    static async invokeFunctionTools(data, { reasoningText = null } = {}) {
        ...
        for (const toolCall of toolCalls) {
            ...
            __stage.console.log('[ToolManager] Function tool call:', toolCall);
            ...
            const toast = message && __stage.toastr.info(message, 'Tool Calling', { timeOut: 0 });
            const toolResult = await ToolManager.invokeFunctionTool(name, parameters);
            __stage.toastr.clear(toast);
            __stage.console.log('[ToolManager] Function tool result:', result);
    ...
    static #formatToolInvocationMessage(invocations) {
        ...
        const preElement = __stage.document.createElement('pre');
        const codeElement = __stage.document.createElement('code');
    ...
    static async saveFunctionToolInvocations(invocations) {
        ...
        const message = {
            name: __stage.systemUserName,
            force_avatar: __stage.system_avatar,
            is_system: true,
            is_user: false,
            ...
            extra: {
                ...
                api: (0, __stage.getGeneratingApi)(),
                model: (0, __stage.getGeneratingModel)(),
            },
        };
        __stage.chat.push(message);
        await __stage.eventSource.emit(__stage.event_types.TOOL_CALLS_PERFORMED, invocations);
        (0, __stage.addOneMessage)(message);
        await __stage.eventSource.emit(__stage.event_types.TOOL_CALLS_RENDERED, invocations);
    ...
    static showToolCallError(errors) {
        __stage.toastr.error('An error occurred while invoking function tools. Click here for more details.', 'Tool Calling', {
            onclick: () => __stage.Popup.show.text('Tool Calling Errors', __stage.DOMPurify.sanitize(errors.map(e => `${e.cause}: ${e.message}`).join('<br>'))),
            timeOut: 5000,
        });
    }
    ...
    static initToolSlashCommands() {
        const toolsEnumProvider = () => ToolManager.tools.map(tool => {
            ...
            return new __stage.SlashCommandEnumValue(toolOpenAI.function.name, toolOpenAI.function.description, __stage.enumTypes.enum, __stage.enumIcons.closure);
        });

        __stage.SlashCommandParser.addCommandObject(__stage.SlashCommand.fromProps({
            name: 'tools-list',
            aliases: ['tool-list'],
            ...
            namedArgumentList: [
                __stage.SlashCommandNamedArgument.fromProps({
                    name: 'return',
                    description: 'The way how you want the return value to be provided',
                    typeList: [__stage.ARGUMENT_TYPE.STRING],
                    defaultValue: 'none',
                    enumList: __stage.slashCommandReturnHelper.enumList({ allowObject: true }),
                    forceEnum: true,
                }),
            ],
            callback: async (args) => {
                ...
                return await __stage.slashCommandReturnHelper.doReturn(returnType ?? 'popup-html', tools ?? [], { objectToStringFunc });
            },
        }));
        ...
        __stage.SlashCommandParser.addCommandObject(__stage.SlashCommand.fromProps({
            ...
            namedArgumentList: [
                __stage.SlashCommandNamedArgument.fromProps({
                    ...
                    typeList: [__stage.ARGUMENT_TYPE.DICTIONARY],
                    isRequired: true,
                    acceptsMultiple: false,
                }),
            ],
            unnamedArgumentList: [
                __stage.SlashCommandArgument.fromProps({
...
/tools-register name=Echo description="Echoes a message. Call when the user is asking to repeat something" parameters={{var::echoSchema}} {: /echo {{var::arg.message}} :}</code></pre>`,
            namedArgumentList: [
                ...
                __stage.SlashCommandNamedArgument.fromProps({
                    ...
                    typeList: [__stage.ARGUMENT_TYPE.CLOSURE],
                    isRequired: true,
                    acceptsMultiple: false,
                }),
                __stage.SlashCommandNamedArgument.fromProps({
                ...
                __stage.SlashCommandNamedArgument.fromProps({
                    ...
                    typeList: [__stage.ARGUMENT_TYPE.BOOLEAN],
                    isRequired: false,
                    acceptsMultiple: false,
            ...
            callback: async (args, action) => {
                ...
                function closureToFunction(action, convertResult) {
                    return async (args) => {
                        ...
                        if (typeof args === 'object' && args !== null) {
                            (0, __stage.assignNestedVariables)(scope, args, 'arg');
                ...
                if (!(action instanceof __stage.SlashCommandClosure)) {
                ...
                if (formatMessage && !(formatMessage instanceof __stage.SlashCommandClosure)) {
                    throw new Error('The "formatMessage" argument must be a closure.');
                }
                if (shouldRegister && !(shouldRegister instanceof __stage.SlashCommandClosure)) {
                    throw new Error('The "shouldRegister" argument must be a closure.');
                }

                const actionFunc = closureToFunction(action, x => x);
                const formatMessageFunc = formatMessage instanceof __stage.SlashCommandClosure ? closureToFunction(formatMessage, x => String(x)) : null;
                const shouldRegisterFunc = shouldRegister instanceof __stage.SlashCommandClosure ? closureToFunction(shouldRegister, x => (0, __stage.isTrueBoolean)(x)) : null;
                ...
                ToolManager.registerFunctionTool({
                    ...
                    stealth: stealth && (0, __stage.isTrueBoolean)(String(stealth)),
                });
...
return { isJson, tryParse, stringify, ToolDefinition, ToolManager };
}
