// TavernStage shared core, extracted from public/scripts/macros/engine/MacroCstWalker.js.
// Upstream 8172dcd0ee672d3cd9a5e5f7af134f91a45cd2b8; AGPL-3.0; source declarations: MacroCstWalker:81
...
class MacroCstWalker {
    ...
    extractMacroInfo(macroNode) {
        ...
        const isClosing = flagTokens.some(token => token.image === __stage.MacroFlagType.CLOSING_BLOCK);
    ...
    #evaluateMacroNode(macroNode, context, scopedContent) {
        ...
        const flags = flagSymbols.length > 0 ? (0, __stage.parseFlags)(flagSymbols) : (0, __stage.createEmptyFlags)();
        ...
        const macroDef = __stage.MacroRegistry.getMacro(name);
            ...
            if (operatorToken) {
                ...
                switch (operatorImage) {
                    ...
                    case '<=':
                        ...
                        break;
                    default:
                        (0, __stage.logMacroInternalError)({ message: `Lexer found macro operator that is not implemented for variable shorthand expressions in macro node '${macroNode.name}'.` });
                        break;
                }
            }
        }
    ...
    #executeVariableOperation(varName, isGlobal, operation, lazyValue) {
        const ctx = __stage.SillyTavern.getContext();
        ...
        * @returns {string}
        */
        const normalize = __stage.MacroEngine.normalizeMacroResult.bind(__stage.MacroEngine);
         ...
         * @returns {boolean}
         */
        const isFalsy = (val) => !val || (0, __stage.isFalseBoolean)(normalize(val));
        ...
        switch (operation) {
            ...
            case 'sub': {
                ...
                else (0, __stage.logMacroRuntimeWarning)({ message: `Variable shorthand "-=" operator requires a numeric value, got: "${lazyValue()}"` });
                return '';
            }
            ...
            case 'greaterThan': {
                ...
                if (isNaN(currentNum) || isNaN(compareNum)) {
                    (0, __stage.logMacroRuntimeWarning)({ message: `Variable shorthand ">" operator requires numeric values. Got: "${vars.get(varName)}" > "${lazyValue()}"` });
                    return 'false';
                }
            ...
            default:
                (0, __stage.logMacroRuntimeWarning)({ message: `Unknown variable shorthand operation: "${operation}"` });
                return '';
        }
    }
    ...
    #evaluateVariableValue(operatorChildren, context) {
        ...
        const startOffset = __stage.Math.min(...allRanges.map(r => r.startOffset));
        const endOffset = __stage.Math.max(...allRanges.map(r => r.endOffset));
    ...
    #evaluateRawContent(rawContent, newContextOffset, context) {
        ...
        const { cst } = __stage.MacroParser.parseDocument(rawContent);
    ...
    #canAcceptScopedContent(macroNode, macroName) {
        const def = __stage.MacroRegistry.getPrimaryMacro(macroName);
        if (!def) {
